...
class AAGorder(BiobbObject):
    ...
    @launchlogger
    def launch(self) -> int:
        """Execute the :class:`AAGorder <gorder.gorder_aa.AAGorder>` gorder.gorder_aa.AAGorder object."""
...
def gorder_aa(input_top_path: str,
              ...
              **kwargs) -> int:
    """Create :class:`AAGorder <gorder.gorder_aa.AAGorder>` class and
    execute :meth:`launch() <gorder.gorder_aa.AAGorder.launch>` method"""
    return AAGorder(**dict(locals())).launch()


gorder_aa.__doc__ = AAGorder.__doc__
main = AAGorder.get_main(gorder_aa, "Compute atomistic lipid order parameters using gorder order tool.")
